...
toRenderObject = flip (GML.foldObject algebra) identityTransformation
    where algebra = ( GML.SimpleTransformer $
                      \shape closure transformation -> Renderer.Simple shape transformation (Evil.shader closure)
...
toWorld (GML.Scene amb l obj dp fov w h fil) = 
       Renderer.World (Renderer.RenderOptions (toColour amb) dp (toRadians fov) w h fil) (toRenderObject obj) l
